...
| model job end to end (toy, rigged, one photo) | 266 s: restyle 86 s, cut-out 2 s, TRELLIS.2 71 s, post 61 s, rig 46 s; 11,421 tris, 5 bones, 1.1 GB on disk (raw TRELLIS output) |
...
7. **Windows line endings broke the profile on the VM.** A copy of `meadowbots-flat.yaml` scp'd with CRLF made svc-post's tiny YAML reader keep inline comments (`.` does not match `\r`), so `target_tris` read as text, the triangle target became NaN, decimation was skipped and Blender baked the full 9 M-triangle mesh for 15 minutes before I stopped it. The reader now strips `\r`, every text file on the VM was converted to LF, and `.gitattributes` pins LF for yaml/json/sh/mjs/ts/py/vue.
8. **The rebuilt comfyui image had lost `pixi`.** comfy-env's `install.py` had put the binary in the container's home directory in Phase 1, which a rebuilt (and now force-recreated) container does not keep, so UniRig registered 0 nodes and the rig stage failed with "Node 'Rig input mesh' not found". `pixi` is now downloaded into the image (`comfyui/Dockerfile`); the 12 GB env itself was always on the bind mount and survived.
