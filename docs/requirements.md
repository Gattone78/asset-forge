...
### Phase 8 — Photo-driven creation (promo clips and rigged models from Phil's photos) — PROPOSED 2026-09-25, Phil to confirm before any download

Purpose: start from an uploaded photo instead of a text prompt, in a chosen visual style: (a) a short promo clip with narration, music and sound effects; (b) a rigged 3D model. Everything reuses the Phase 2–7 stages; the new pieces are uploads, a restyle step and two pipeline jobs that chain the stages inside one job.

- **Uploads:** `POST /uploads` (multipart; PNG/JPG/WebP, ≤ 20 MB each, several per call) stores `/srv/forge/uploads/<uuid>.<ext>` plus a small JSON (original name, size, sha256, pixel size, uploaded_at); `GET /uploads` lists, `GET /uploads/<uuid>` serves, `DELETE /uploads/<uuid>` removes (refused while a job references it). Jobs reference uploads as `upload:<uuid>` wherever an image is accepted (`video.init_image`, the new fields below). CLI: `forge upload <file…>` prints ids. UI: a file picker with thumbnails in the new-job dialog and an Uploads page. Uploads are not job data: they survive job deletion and are part of the backup set (§9).
- **Styles** (profile `styles:` block, extensible in YAML; a `style` field on the new jobs): `realistic` (no restyle; "photorealistic, natural light" prompt suffix), `pixar` ("3D animated feature film look, soft subsurface skin, big expressive eyes, rounded shapes"), `2d` ("flat 2D cartoon, clean line art, cel shading"), `toy` (= the meadowbots-flat look). Each style carries: a restyle prompt, a video prompt suffix, a 3D prompt suffix, and music/sfx hints.
- **Restyle model:** **Qwen-Image-Edit 2511** (Apache-2.0; ComfyUI core `TextEncodeQwenImageEditPlus`; Comfy-Org repackage: 20.5 GB fp8 diffusion model + Qwen2.5-VL 7B text encoder ≈ 9 GB + VAE). The photo goes in as the edit reference and comes out in the style at the video/3D input size; identity and composition are preserved by the model. FLUX.1 Kontext dev was rejected on licence (non-commercial). Same Blackwell gate as every model: `workflows/test-restyle.json` — a test photo → `pixar` at seed 1 must produce an image that is not the input (SSIM below a threshold) with the subject still centred; record VRAM peak and wall time.
- **Job `promo`:** `{photo: upload id, prompt, style, profile, seed, duration_s (5), aspect, script? (narration text), narration_at_s (default 1.0), music (default true), music_prompt?, foley (default true), title?}` → stages in one job: **restyle** (skipped for `realistic`) → **video** (Wan 2.2 I2V from the styled still; prompt + style suffix) → **foley** (MMAudio on the clip) → **speech** (Kokoro, only with a script) → **music** (ACE-Step, duration ≥ clip, loop off) → **mix** (svc-post trailer mode: optional title card, clip audio + ducked music + narration) → `out/<name>.mp4` + poster + the intermediates (`styled.png`, silent clip, tracks) + one sidecar listing every stage. UI: the final MP4 with the styled still beside the original; approve/reject like any asset. CLI: `forge job promo --photo <upload id> "prompt" --style pixar [--script "…"] […]`.
- **Job `model`** (rigged 3D from photos): `{photos: [upload ids], prompt, style, profile, seed, rig (default true), height_m?}` → **restyle** per photo (skipped for `realistic`) → **cutout** (BiRefNet, existing) → **3D** (TRELLIS.2 per photo = one candidate each; the existing candidate selection picks the best, and the UI lets Phil pick another) → **post** → **rig** (UniRig; template `articulationxl` for creatures/animals, the humanoid template for people — decided at the gate with a photo of a person) → GLB/FBX as in Phases 2 and 4, viewer with bones. **Several photos in v1 mean several candidates, not one fused model**: core TRELLIS.2 is single-view. True multi-view is an option for later: Hunyuan3D 2.1 multi-view nodes are in ComfyUI core (`Hunyuan3Dv2ConditioningMultiView`) and would get their own gate; the Qwen-Edit "multiple angles" LoRA could synthesise side/back views from one photo.
- **Post/UI/CLI/docs** as in every phase; `docs/phase-8.md` with the gate numbers, the face-fidelity caveat (a single photo gives a plausible stylised head, not a likeness), and the pixel-size/orientation rules for photos (EXIF rotation is applied at upload; faces should fill ≥ 25 % of the frame).

**Accept:** in the UI, upload a photo, choose `pixar`, type a prompt and a one-line script → one MP4 with narration, music and foley reaches review and plays on a phone; upload one photo + prompt → a rigged GLB with bones in the viewer; the same photo with `realistic` gives a photoreal clip without the restyle step; `comfyui` stops on idle; uploads survive job deletion and are included in the backup set.

**Decisions for Phil before download:** (1) Qwen-Image-Edit 2511 fp8 as the restyle model (recommended; ~30 GB more on the data disk); (2) multi-photo = candidates in v1, multi-view fusion deferred (recommended) or a Hunyuan3D 2.1 multi-view gate in this phase; (3) narration script typed by Phil (recommended) rather than drafted by an LLM (would need a text model on the box; big-cat's vLLM is out of scope); (4) the initial style list (`realistic`, `pixar`, `2d`, `toy`).

### Later (not v1, do not build yet)

Multi-view 3D fusion (Hunyuan3D 2.1 multi-view or a TRELLIS multi-view node), voice cloning, an LLM for narration scripts, MCP server over the REST API, runtime generation, Kubernetes deployment, multi-GPU scheduling.
